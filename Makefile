...
#     '<,'>g/-schema/m'<-1
.PHONY: read
read: read_documents read_donations read_donees read_donors read_gifts read_money_moved read_disclosures read_donor_donee_relationships reset_cache build_cache
...
.PHONY: reset_ftx_grants
reset_ftx_grants:
	mysql $(MYSQL_ARGS) $(DATABASE) -e "delete from donations where donor = 'FTX Future Fund';"

.PHONY: reset_open_phil_grants
